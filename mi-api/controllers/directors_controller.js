...
export const searchDirectors = async (req, res) => {
  ...
  try {
    const directors = await Directors.find({ surname: { $regex: surname, $options: 'i' }, status: true });
    res.json(directors);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Error al buscar directores por apellido' });
  }
};
...
export async function deactivateDirector(id) {
  let deactivatedDirector = await Directors.findByIdAndUpdate(id, {
      $set: {
          status: false
      }
  }, { new: true })
...
export const updateDirector = async (req, res) => {
  ...
  try {
      const updatedDirector = await Directors.findByIdAndUpdate(id, { surname, age }, { new: true });
